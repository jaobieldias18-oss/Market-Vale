...
export default function AdminCategorias() {
  ...
  async function handleDelete(id: string) {
    const ok = confirm("Excluir esta categoria? Os negÃ³cios dela ficarÃ£o sem categoria.");
    if (!ok) return;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      <p className="mt-1 text-sm text-slate-500">Tipos de negÃ³cio disponÃ­veis no catÃ¡logo</p>
      ...
      <form onSubmit={handleCreate} className="mt-6 grid gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:grid-cols-2">
        ...
        <input className="input" placeholder="Ãcone (emoji)" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} />
        ...
        <div className="sm:col-span-2">
          <textarea className="input resize-none" rows={2} placeholder="DescriÃ§Ã£o curta" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
        </div>
            ...
            {editingId === cat.id ? (
              ...
              <>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-slate-400">/{cat.slug} Â· {cat.description}</p>
                </div>
